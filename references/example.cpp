...
void f(int &&) {
    cout << "- f() got called with an R-value reference, ready to MOVE" << endl;
}

void f(const int &) {
    cout << "- f() got called with an L-value reference, ready to COPY" << endl;
}
...
class TestClass {
 public:
    ...
    TestClass(const TestClass &) {
        cout << "- Matched COPY constructor with L-value argument" << endl;
    }

    TestClass(TestClass &&) {
        cout << "- Matched MOVE constructor with R-value argument" << endl;
    }
